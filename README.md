...
|-------|------------------------|-------|
| **DNS Cache** | دومین اتصال به google.com:<br/>بدون کش: ~150ms DNS<br/>با کش: ~0ms | ~150ms صرفه‌جویی |
| **حذف XHTTP_LOCK از hot path** | هر چانک دانلینک یکبار lock acquire و release می‌کرد = overhead اضافه<br/>در asyncio لازم نیست | ~۵‑۱۰٪ سریع‌تر |
| **سوکت تیونینگ در relay_vless** | relay_vless بدون `TCP_NODELAY` و `SO_SNDBUF` بود = هر پکت منتظر ACK می‌موندست | ~۱۰‑۲۰٪ سریع‌تر |
| **BUFFER ۲۵۶→۵۱۲KB در relay_vless** | چانک بزرگ‌تر = syscall کمتر = overhead کمتر | ~۵٪ |
| **کش ساعت (strftime)** | `strftime` تابع سنگینه، هر چانک اجرا می‌شد → الان هر ساعت یکبار | ~۲‑۳٪ |
| **save_state محدود** | هر اتصال جدید دیسک I/O داشت → الان هر ۳۰ ثانیه | I/O کمتر |
| **آپدیت dependencies** | uvicorn 0.32 سرعت اولیه بالاتر، fastapi 0.115 بهینه‌تر | ~۵‑۱۰٪ |
